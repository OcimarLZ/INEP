...
from inep_models import (Uf, Regiao, Mesorregiao, Microrregiao, Municipio, Tp_organizacao_academica, Tp_categoria_administrativa,
                         Mantenedora, Ies, Ies_censo, Tp_rede)
...
de_para_cadbase = {'NU_ANO_CENSO': 'ano_censo',
     'NO_REGIAO_IES': 'nome',
     ...
     'NU_CEP_IES': 'cep'}
de_para_cadies = {'NU_ANO_CENSO': 'ano_censo',
     'CO_REGIAO_IES': 'regiao',
     'SG_UF_IES': 'estado',
     'CO_MUNICIPIO_IES': 'municipio',
     'CO_MESORREGIAO_IES': 'mesorregiao',
     'CO_MICRORREGIAO_IES': 'microrregiao',
     'TP_ORGANIZACAO_ACADEMICA': 'org_academica',
     'TP_CATEGORIA_ADMINISTRATIVA': 'categoria',
     'CO_MANTENEDORA': 'mantenedora',
     'CO_IES': 'ies'}
encode = 'ISO-8859-1'
delimitador = ';'
df = pd.DataFrame()
# Definir o caminho da pasta
anos = range(2009, 2023 + 1)
for ano in anos:
...
uf_df = df[['NO_UF_IES', 'SG_UF_IES', 'CO_UF_IES', 'CO_REGIAO_IES']].drop_duplicates()
...
# Remove as colunas não previstas no bancfo de dados
bd_df = bd_df.drop(columns='TP_REDE')
bd_df = bd_df.drop(columns='in_comunitaria')
bd_df = bd_df.drop(columns='in_confessional')
